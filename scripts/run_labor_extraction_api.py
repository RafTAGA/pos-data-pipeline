"""
Runner for Toast labor (time entries) extraction, driven entirely by
...
Optional:
    ...
    OUTPUT_DIR   (default: raw_labor/<start year>)
    ...
    labor_time_entries in Neon, through today.
...
def main() -> None:
    ...
    start_date, end_date = _resolve_date_range("labor_time_entries")

    default_output_dir = REPO_ROOT / "raw_labor" / str(start_date[0])
    output_dir = os.environ.get("OUTPUT_DIR", str(default_output_dir))

    print(f"Running Toast labor extraction: {start_date} -> {end_date}")
    ...
    #### Checkpoint: confirm the 29-day-chunk JSON files were actually written,
    #### and show a quick preview — without ever touching/printing credentials or tokens.
    chunk_files = sorted(glob.glob(os.path.join(output_dir, "time_entries_*.json")))

    if not chunk_files:
        print(f"\nCHECKPOINT: no time_entries_*.json files found in {output_dir} "
    ...
    for path in chunk_files:
        ...
        print(f"\nCHECKPOINT [{os.path.basename(path)}]: {len(entries)} time entries saved.")

    print(f"\nCHECKPOINT TOTAL: {len(chunk_files)} chunk file(s), {total_entries} time entries across the range.")
    with open(chunk_files[0], "r", encoding="utf-8") as f:
        entries = json.load(f)
    if entries:
        print("\nFirst time entry (preview):")
        print(json.dumps(entries[0], indent=2)[:1500])

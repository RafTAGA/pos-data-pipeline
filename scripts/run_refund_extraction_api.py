"""
Runner for Toast refund extraction, driven entirely by environment variables
...
Optional:
    ...
    OUTPUT_DIR   (default: raw_refunds/<start year>)
    ...
    refunds_daily_summary in Neon, through today.
...
def main() -> None:
    ...
    start_date, end_date = _resolve_date_range("refunds_daily_summary")

    default_output_dir = REPO_ROOT / "raw_refunds" / str(start_date[0])
    output_dir = os.environ.get("OUTPUT_DIR", str(default_output_dir))

    print(f"Running Toast refund extraction: {start_date} -> {end_date}")
    ...
    refund_files = sorted(glob.glob(os.path.join(output_dir, "refunds_*.json")))

    if not refund_files:
        print(f"\nCHECKPOINT: no refunds_*.json files found in {output_dir} "
              f"(extraction may have failed -- check logs above).")
        return

    total_payments = 0
    for path in refund_files:
        with open(path, "r", encoding="utf-8") as f:
            payments = json.load(f)
        total_payments += len(payments)
        print(f"\nCHECKPOINT [{os.path.basename(path)}]: {len(payments)} refunded payments saved.")

    print(f"\nCHECKPOINT TOTAL: {len(refund_files)} file(s), {total_payments} refunded payments across the range.")
    with open(refund_files[0], "r", encoding="utf-8") as f:
        payments = json.load(f)
    if payments:
        print("\nFirst refunded payment (preview):")
        print(json.dumps(payments[0], indent=2)[:1500])
